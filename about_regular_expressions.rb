...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    # THINK ABOUT IT:
    #
    # When would * fail to match? when used on nil?

    # Assuming failure means returning nil, it seems that * will never fail to match because its design to return an empty string even when there are no matches in the target string. https://stackoverflow.com/questions/5084141/getting-the-star-regex-operator-to-fail
  end
  ...
  #
  # Why?
  # The operators are greedy because as many occurrances as possible are matched
  # ------------------------------------------------------------------
  ...
  def test_character_classes_give_options_for_a_character
    ...
    assert_equal ["cat", "bat", "rat"], animals.select { |a| a[/[cbr]at/] }  #select = filter
  end
  ...
  # When using a character class to match the target string, each regexp token is compared to the current string character before moving along the string. Whereas with alternation, the first token is compared to every string character before moving on to the next token
  # ------------------------------------------------------------------
